...
[Релизы](docs/RELEASES.md) · [История изменений](docs/CHANGELOG.md)

[Безопасность](docs/SECURITY.md) · [Участие в разработке](docs/CONTRIBUTING.md) · [Архитектура](docs/ARCHITECTURE.md) · [Совместимость](docs/COMPATIBILITY.md) · [Чеклист релиза](docs/RELEASE_CHECKLIST.md)
...
На совместимом OpenWrt текущий закреплённый релиз можно установить одной
POSIX-совместимой командой:

```sh
wget -qO- 'https://github.com/unisequence/freenetic/releases/latest/download/install.sh' | sh
```

Release installer генерируется из финальной матрицы пакетов: в него
записываются точные SHA-256 для четырёх LuCI-пакетов и бинарника `fnc`,
собранного под ABI выбранного package manager. Билдер также коммитит этот
файл в release tag, поэтому старый raw-URL
получает те же актуальные хеши. Installer сначала проверяет роутер,
автоматически выбирает APK или IPK, сохраняет установленную версию релиза и
ставит `fnc` в `/usr/bin/fnc`.
Русские пакеты устанавливаются, но текущий язык LuCI сам не переключается.
...
- Живые метрики Dashboard и Traffic Monitor запрашиваются короткими пакетами
  через нативный ubus endpoint uhttpd, не удерживая CGI worker открытым
- Internet (мульти-WAN)
