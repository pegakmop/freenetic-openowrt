...
try {
	...
	for (const arch of [ 'aarch64_cortex-a53', 'mipsel_24kc' ])
	...
	assert.equal(result.assetCount, 27, 'release must contain core packages, Zapret2 builds and keys, binaries, installer and manifest');
	...
	assert.equal(manifest.trim().split('\n').length, 26, 'manifest must cover every pre-manifest asset');
